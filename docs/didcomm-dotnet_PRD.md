...
| FR-API-04 | MUST | `UnpackResult` exposes metadata: encrypted?, authenticated?, non-repudiation?, anonymous-sender?, enc alg, kw alg, sig alg, signer kid, sender kid, recipient kids, `from_prior` (if any), and the detected envelope stack — plus, on the provenance-capable key-service path, the same-document `VerifiedKeyBinding` evidence for sender/signer/recipient (FR-CONSIST-07): non-positional, public-get/internal-init properties so external callers can read but never forge them, and additive over the v1.3.0 positional constructor (binary compatible). | Metadata correct for each composition; bindings present iff the key service implements `IDidKeyBindingService`. |
