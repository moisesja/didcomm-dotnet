...
| FR-PROTO-12 | MUST | **Inbound observer seam**: provide `IProtocolObserver` that the dispatcher delivers every matching inbound message to, for every completed dispatch (all outcomes, including `NoHandler` and loop-guard drops), so a second consumer can observe traffic whose PIURI is owned by a built-in handler (e.g. react to inbound `report-problem`, correlate a Discover Features `disclose`) without replacing that handler. **Execution model:** delivery MUST be decoupled from dispatch — the dispatcher enqueues onto a per-observer bounded background queue and returns the outcome immediately, so a slow/hung/faulting observer can neither gate reply delivery nor change the outcome, and one hung observer cannot starve another; an over-full observer queue drops the newest observations (logged), never grows unbounded. Delivery is therefore **best-effort / at-most-once**, not guaranteed. No observer code (including the `ProtocolUriFilter` getter) runs on the dispatch path: the filter is read ONCE, guarded, at construction (a throwing getter disables that observer). Observers may be invoked concurrently and MUST be thread-safe. **Threat model:** the seam is host-trusted and read-only. Observers (a) are registrable ONLY at DI-composition time (`AddProtocolObserver<T>()`), never at runtime and never by a remote peer; (b) run off the dispatch path, so they cannot influence handler resolution, replies, or thread state; (c) receive a defensive deep clone of the message plus envelope-auth metadata via `InboundObservation` — so mutation through the payload cannot reach the pipeline, a handler, a reply, or another observer. This is message-mutation isolation, NOT a capability sandbox: observers are trusted in-process host code and can inject `DidCommClient`/thread-state themselves (the built-in Discover Features client does) — omitting the facade from the payload keeps the surface minimal, it does not prevent host code from acting; (d) are scoped by a per-PIURI `ProtocolUriFilter` (least privilege; `null` = all); (e) are enumerated in an Information log line at dispatcher construction (operator-auditable against a stowaway registration). Observer exceptions are isolated and logged; the dispatch outcome is unaffected. Envelope cryptography is untouched. | Observer sees handled + `NoHandler` + dropped inbounds; a never-completing OR filter-throwing observer neither blocks dispatch nor starves another nor breaks construction; a cancelled dispatch cannot clobber the outcome; an over-full queue drops rather than grows; a mutating observer cannot affect the handler input, the reply, or another observer's view; a filtered observer sees only its protocol family. |
