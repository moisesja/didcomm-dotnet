...
| FR-CONSIST-08 | MUST | **Decrypted recipient entry ↔ its label.** The recipient `kid` reported by an encrypt layer is the label of whichever recipient entry the derived KEK opened, and those labels are attacker-authored (`apv` commits only to the sorted kid list, so labels can be permuted without invalidating the envelope). The reported recipient `kid` MUST equal the kid whose key agreement produced the KEK; otherwise reject with `ConsistencyException`. Without this, an envelope whose real key-wrap is labelled with another DID's kid decrypts with the local key while `RecipientKid` — and any evidence derived from it — names a key that did not decrypt it. | **Negative:** a two-recipient JWE with the two entries' `kid` labels swapped ⇒ rejected, even though the ciphertext, tag, and `apv` are untouched. **Positive:** the same envelope unswapped unpacks and reports the local kid. |
