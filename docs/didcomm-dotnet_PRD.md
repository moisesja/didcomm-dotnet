...
| FR-PROTO-05a | MUST | **Discover Features 2.0 (initiator)**: provide an async round-trip that sends a `queries` and awaits the correlated `disclose` — `QueryFeaturesAsync(from, to, queries, timeout, …)`. Correlate the reply by `thid` == the query `id`; the `timeout` bounds the whole operation (send + await) and a transport failure MUST propagate unchanged (not be relabeled a timeout). A `disclose` MUST complete a pending query ONLY when the envelope authenticates the sender (authcrypt or verified signature), carries a sender/signer key id, AND its `from` is the queried DID **compared as a DID subject (§4.3)**; an unauthenticated, keyless, or wrong-sender disclosure MUST NOT complete (or cancel) the waiter. Correlation runs as a **lossless, synchronous, internal `IInboundCorrelator`** (invoked inline by the dispatcher, guarded), NOT via the best-effort observer queue — so a genuine authenticated response completes even under a flood of unsolicited disclosures, and there is no default observer/firehose. The responder-side handler (FR-PROTO-05) is unchanged — an inbound `disclose` remains a terminal leaf in dispatch. **Reply delivery is the responder application's job**, out-of-band per FR-TRN-10 (`DidCommClient.SendAsync` to the initiator's receive endpoint, bound to the authenticated inbound sender): the library ships NO automatic inbound-triggered egress (an auto-send that trusts the message's from/endpoint is a reflector/oracle/DoS hazard). | Two-agent HTTP round-trip returns correlated disclosures with no manual injection (responder app delivers the disclose); timeout throws; a transport failure surfaces as itself; forged (unauthenticated / keyless / wrong-subject) disclose is ignored; real authcrypt + signed disclosures complete, tampered ones fail signature/AEAD at unpack. |
...
| FR-PROTO-12 | MUST | **Inbound observer seam**: provide `IProtocolObserver` that lets an application observe inbound traffic whose PIURI is owned by a built-in handler (e.g. react to inbound `report-problem`) without replacing that handler. **Opt-in only:** `AddBuiltInProtocols()` registers NO default observer — the Discover Features correlation uses a separate lossless inline `IInboundCorrelator` (FR-PROTO-05a), not this queue — so a default deployment has no observer/firehose surface for a remote flood to target. **Execution model:** delivery MUST be decoupled from dispatch — the dispatcher enqueues onto a per-observer bounded background queue and returns the outcome immediately, so a slow/hung/faulting observer can neither gate reply delivery nor change the outcome, and one hung observer cannot starve another. Each queue is bounded by BOTH an item count AND a byte budget; on overflow the newest observation is dropped with **rate-limited** logging — so a flood cannot exhaust memory or amplify logging. Delivery is therefore **best-effort / at-most-once**, not guaranteed. Each observation is **snapshotted (cloned) at enqueue**, so a handler/caller mutating the live message after dispatch cannot change what an observer sees. No observer code (including the `ProtocolUriFilter` getter) runs on the dispatch path: the filter is read ONCE, guarded, at construction (a throwing getter disables that observer). Pumps honor a shutdown token cancelled on disposal. Observers may be invoked concurrently and MUST be thread-safe. **Threat model:** the seam is host-trusted and read-only. Observers (a) are registrable ONLY at DI-composition time (`AddProtocolObserver<T>()`), never at runtime and never by a remote peer; (b) run off the dispatch path; (c) receive a defensive deep clone via `InboundObservation` — mutation through the payload cannot reach the pipeline, a handler, a reply, or another observer. This is message-mutation isolation, NOT a capability sandbox: observers are trusted in-process host code and can inject `DidCommClient`/thread-state themselves — omitting the facade keeps the surface minimal, it does not sandbox host code; (d) are scoped by a per-PIURI `ProtocolUriFilter`; (e) are enumerated in an Information log line at construction. Observer exceptions are isolated and logged. | No default observer is registered; observer sees handled + `NoHandler` + dropped inbounds; a never-completing OR filter-throwing observer neither blocks dispatch nor starves another nor breaks construction; a cancelled dispatch cannot clobber the outcome; an over-full queue (by count OR bytes) drops rather than grows; a snapshot is unaffected by later mutation of the live message; disposal stops pumps even with a hung observer. |
