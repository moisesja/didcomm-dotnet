...
| FR-CONSIST-04 | SHOULD | **Recipient self-presence in `to` (#59).** When a `to` header is present, verify one of the recipient's **own identifiers** appears (compared as DID subjects). "Own" is the union of the decrypting recipient kid's DID subject (encrypted envelopes) and the DIDs the application declares in `DidCommOptions.OwnIdentifiers` — the spec's "their own identifier" is the agent's identity, which only the application can assert for non-encrypted receives. If none appears, do NOT fail: surface the warning as data — `UnpackResult.RecipientAddressing = NotAddressed` (`Addressed` on a match; `NotEvaluated` when `to` is absent or no parseable own identifier is known). An unparseable configured identifier is skipped by the check, never counted as "checked and absent" (and rejected at `DidCommClient` construction so a typo cannot silently disable the signal); `OwnIdentifiers` is snapshotted at construction so a concurrent caller mutation cannot fault an unpack. Note the encrypted mismatch case is already **rejected** by the stronger FR-CONSIST-02 MUST, so the warning is reachable primarily on signed-only and plaintext receives — exactly the paths with no addressing check otherwise. **Per-message cost MUST be bounded by the wire input, not by the host's declared roster:** `OwnIdentifiers` MUST be normalized to DID subjects, deduplicated, and frozen once at construction, and the check MUST then walk only the `to` sequence against that prebuilt set — never enumerating it — and MUST NOT exit early on a match. A per-message loop over the configured identifiers instead costs `O(|to| × |own|)`, letting unauthenticated traffic amplify CPU by a receiver-chosen factor and exposing roster scale through latency; a short-circuit additionally lets a sender time a large `to` list to learn whether a guessed DID is one of the agent's. Neither property is a constant-time guarantee (hash lookups and string equality are data-dependent); the requirement is that the dominant, network-visible cost carry no such signal. `Addressed` is not a trust signal: `to` is attacker-authored, so it is meaningful only on an authenticated or signed envelope. | **Positive:** signed-only message with `to = [did:example:carol]` and `OwnIdentifiers = [did:example:bob]` ⇒ delivered with `NotAddressed`; same message with bob in `to` ⇒ `Addressed`. **Negative-space:** unconfigured agents see `Addressed`/`NotEvaluated` only; delivery is never affected. |
